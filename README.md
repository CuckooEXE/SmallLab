...
| **Nexus**      | `https://packages.lab`, `https://docker.lab`    | via Caddy                    | [Sonatype Nexus Repository OSS](https://help.sonatype.com/en/sonatype-nexus-repository.html) — universal package registry (Artifactory-style). `bootstrap.sh` creates one hosted repo per format — npm, PyPI, cargo, Go, raw (tarballs), yum, apt (Debian) — on `packages.lab`, plus Docker/OCI on `docker.lab`. Add proxy/mirror + group repos in the UI if you want upstream caching |
